...
public class AgentWin implements INotifyObject {
	...
	private void send1() {
		try {
			...
			boolean f = combo.getText().toString().trim().toUpperCase()
					.equals("FA");
			if (f) {
	...
	void selectclient() {
		if (list != null) {
			if (list.getSelection().length > 0) {
				...
				if (text_1 != null)
					text_1.setText(client);
				System.out.println(""
						+ client.substring(0, client.indexOf("/")));

				String txt = text_1.getText().toString().trim();
				showMessage(txt.substring((txt.indexOf("[") + 1),
						txt.indexOf("]")));
			}
		}
	}
	...
	public static void main(String[] args) {
		try {
			JobAgent jobAgent = new JobAgent("10.64.145.245", "Hui-PC",
					"agent", "bdesdk", ""); // Compaq-PC
	...
	class ChatTip implements Runnable {
		...
		public void run() {
			...
			System.out.println("chats on ui:" + v.size());
			if (list != null) {list.removeAll();
				for (int i = 0; i < v.size(); i++) {
					...
					try {
						...
						if (ot0 != null) {
							if (ot0.getIsactive())list.add(JobAgent.chatProcess.getCMC(threadid).getParticipant()+ "["+ threadid+ "]");
						} else {
	...
	public void open() {
		...
		text_2.setText("1234");
		GridData gd_text_2 = new GridData(SWT.FILL, SWT.CENTER, true, false, 1,
				1);
