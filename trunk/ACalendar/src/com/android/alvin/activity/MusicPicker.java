...
public class MusicPicker extends ListActivity {
	...
	class MusicListAdapter extends BaseAdapter{
		...
		public MusicListAdapter(Context context) {
			...
			if(mCursor!=null){
				...
				artistIdx = mCursor.getColumnIndex(MediaStore.Audio.Media.ARTIST);
				durationIdx = mCursor.getColumnIndex(MediaStore.Audio.Media.DURATION);
			}
			mCursor.moveToFirst();
		}
		...
		public View getView(int position, View view, ViewGroup parent) {
			...
			if(album==null||album.equals(""))album = UNKOWN_ALBUM;
			if(artist==null||artist.equals(""))artist = UNKOWN_ARTIST;
	...
	public void onCreate(Bundle bundle){
		...
		/*lv.setOnItemClickListener(new ListView.OnItemClickListener(){
			...
			public void onItemClick(AdapterView<?> arg0, View v, int position,
					long arg3) {
				MusicListAdapter.ViewHolder vh = (MusicListAdapter.ViewHolder)v.getTag();
	...
	void doQuery(){
		...
		mCursor = getContentResolver().query(baseUri, CURSOR_COLS, where.toString(), keywords, MediaStore.Audio.Media.TITLE_KEY);
	}
